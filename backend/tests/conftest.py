...
@pytest.fixture
def sample_home_file(db_session,  sample_account):
    import uuid
    from app.models.models.folder import Folder
    folder = Folder(
        uuid = str(uuid.uuid4()),
...
@pytest.fixture
def sample_file(db_session, sample_account, sample_home):
    import uuid
    uuid
    ...
    file = File(
        uuid = str(uuid.uuid4()),
        owner_id = sample_account.id,
        parent_folder_id = sample_home.uuid,
        name = "test.txt",
        size = 1024,
...
def assert_assert_error(resp: dict, code: int) -> None:
